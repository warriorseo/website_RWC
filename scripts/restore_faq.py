...
if old_faq_h2:
    ...
    if not new_soup.find(lambda tag: tag.name == 'h2' and 'FAQ' in tag.get_text()):
        # 2. Insert them into new_soup right before the Summary H2 row
        ...
        if summary_h2:
            ...
            if new_accordion:
                ...
                # Now, re-fetch items because some were decomposed
                items = new_accordion.find_all('div', class_='accordion-item')
                
                # 4. Remove the FIRST FAQ question as requested by user
                if items:
                    print(f"Removing first FAQ item: {items[0].find('span').get_text(strip=True)}")
                    items[0].decompose()
                    items = items[1:]
                    
                # 5. Apply the simplified toggle UI
                for item in items:
