import os
from google.oauth2 import service_account
from google.analytics.data_v1beta import BetaAnalyticsDataClient
from google.analytics.data_v1beta.types import (
    DateRange, Dimension, Metric, RunReportRequest, FilterExpression, FilterExpressionList, Filter,
)

creds_info = {
    'type': 'service_account',
    'project_id': 'gen-lang-client-0042204106',
    'private_key_id': '800e02ba1d02083ec1711ff5f6e033618d050e84',
    'private_key': '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
    'client_email': 'aiwar-841@gen-lang-client-0042204106.iam.gserviceaccount.com',
    'token_uri': 'https://oauth2.googleapis.com/token',
}

creds = service_account.Credentials.from_service_account_info(creds_info)
ga4_client = BetaAnalyticsDataClient(credentials=creds)
property_id = "292925407"
slug = "vagina-filler"

# Let's query by userAgeBracket
print("--- Age Group breakdown (Organic) ---")
req_age = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="yearMonth"), Dimension(name="userAgeBracket")],
    metrics=[Metric(name="activeUsers")],
    date_ranges=[DateRange(start_date="2026-03-01", end_date="2026-06-30")],
    dimension_filter=FilterExpression(
        and_group=FilterExpressionList(
            expressions=[
                FilterExpression(filter=Filter(field_name="pagePath", string_filter=Filter.StringFilter(value=f"/{slug}/"))),
                FilterExpression(filter=Filter(field_name="sessionDefaultChannelGroup", string_filter=Filter.StringFilter(value="Organic Search")))
            ]
        )
    )
)
try:
    res = ga4_client.run_report(req_age)
    for row in res.rows:
        ym = row.dimension_values[0].value
        age = row.dimension_values[1].value
        users = row.metric_values[0].value
        print(f"  Month: {ym} | Age Group: {age:<10} | Users: {users:<4}")
except Exception as e:
    print(f"  Could not retrieve age data: {e}")

# Let's query by userGender
print("\n--- Gender breakdown (Organic) ---")
req_gender = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="yearMonth"), Dimension(name="userGender")],
    metrics=[Metric(name="activeUsers")],
    date_ranges=[DateRange(start_date="2026-03-01", end_date="2026-06-30")],
    dimension_filter=FilterExpression(
        and_group=FilterExpressionList(
            expressions=[
                FilterExpression(filter=Filter(field_name="pagePath", string_filter=Filter.StringFilter(value=f"/{slug}/"))),
                FilterExpression(filter=Filter(field_name="sessionDefaultChannelGroup", string_filter=Filter.StringFilter(value="Organic Search")))
            ]
        )
    )
)
try:
    res = ga4_client.run_report(req_gender)
    for row in res.rows:
        ym = row.dimension_values[0].value
        gender = row.dimension_values[1].value
        users = row.metric_values[0].value
        print(f"  Month: {ym} | Gender: {gender:<10} | Users: {users:<4}")
except Exception as e:
    print(f"  Could not retrieve gender data: {e}")
