...
print("pagePath CONTAINS '/vagina-filler/' (with slashes):")
for row in res.rows:
    users = int(row.metric_values[0].value)
    dur = float(row.metric_values[1].value)
    print(f"  Users = {users}, Avg Eng = {dur/users:.2f}s (Total duration = {dur})")

# Let's list the details to see what paths match
req_details = RunReportRequest(
    property=f"properties/{property_id}",
...
res_details = ga4_client.run_report(req_details)
print("\nDetails:")
for row in res_details.rows:
    path = row.dimension_values[0].value
    users = int(row.metric_values[0].value)
    dur = float(row.metric_values[1].value)
    print(f"  Path: {path:<40} | Users = {users:<4} | Avg Eng = {dur/users:.2f}s")
