...
def fetch_ytd_data():
    ...
    print(f"Fetching YTD Data: {ytd_start} to {ytd_end}")
    print(f"Fetching Previous YTD Data: {prev_ytd_start} to {prev_ytd_end}")
...
def main():
    try:
        ...
        # L: GA4 Active Users YoY (YTD)
        # M: GA4 Engagement Time YoY (YTD)
        # N: GA4 Engagement Rate YoY (YTD)
        col_l = [["GA4 Active Users YoY\n(YTD)"]]
        col_m = [["GA4 Engagement Time YoY\n(YTD)"]]
        col_n = [["GA4 Engagement Rate YoY\n(YTD)"]]
        ...
        for i, row in enumerate(rows):
            ...
            col_m.append([time_sum])
            col_n.append([eng_sum])

        # Also update Column O header to YTD
        col_o_header = [["GA4 Link Compare\n(YTD)"]]
        
        print(f"Updating Columns L, M, N, O for {len(col_l)-1} rows...")
        ...
        body = {
            ...
            "data": [
                ...
                {"range": f"'{target_sheet_name}'!O1:O1", "values": col_o_header}, # Only update header of O
            ]
        }
