...
def format_combined_cell(path, curr_data, past_data):
    ...
    # Line 1: Current YTD (Changed R: to E:)
...
def main():
    try:
        ...
        # New Column L: GA4 YoY Combined Summary (YTD)
        # Header includes explanations
        header_text = "GA4 YoY Combined Summary\n(YTD)\n[A: Active User | T: Time on Page | E: Avg. Engagement Rate]"
        col_l = [[header_text]]
        ...
        for i, row in enumerate(rows):
            ...
            col_l.append([combined_val])

        print(f"Updating Column L for {len(col_l)-1} rows...")
        sheet.values().update(
