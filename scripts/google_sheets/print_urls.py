...
def main():
    ...
    # Read Col A (URLs), Col K (GA4 Link), Col O (GA4 Link Compare)
    result = sheet.values().get(spreadsheetId=sheet_id, range=f"'{target_sheet_name}'!A1:P5", valueRenderOption='FORMULA').execute()
    rows = result.get('values', [])
    for idx, r in enumerate(rows):
        print(f"Row {idx+1}:")
        if len(r) > 0: print(f"  Col A: {r[0]}")
        if len(r) > 10: print(f"  Col K: {r[10]}")
        if len(r) > 14: print(f"  Col O: {r[14]}")
