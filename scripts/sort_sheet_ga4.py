...
# Sort data_rows by column index 1 (Avg Users) descending
data_rows.sort(key=lambda x: float(x[1]) if len(x) > 1 and x[1].replace('.','',1).isdigit() else 0, reverse=True)

sorted_values = [header] + data_rows

# 2. Overwrite sheet with sorted data
body = {
...
print("Successfully sorted the rows.")
