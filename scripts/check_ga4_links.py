...
result = service.spreadsheets().values().get(
    spreadsheetId='1Iz97U0hcmSnyoZVVEA1SxxI3JkImbm4BgAlC_PkWypQ',
    range="'URL Revision and Engagement'!A1:Z20",
    valueRenderOption="FORMULA"
).execute()
values = result.get('values', [])
for i, row in enumerate(values):
    print(f"Row {i+1}: URL={row[0] if row else 'Empty'}, GA4 Link={row[-1] if row else 'Empty'}")
