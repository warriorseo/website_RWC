...
latest_month = f"{today.year}-{today.month:02d}"

start_date = f"{latest_month}-01"
end_date = (today - datetime.timedelta(days=2)).strftime('%Y-%m-%d')

print(f"Fetching Average Position for {start_date} to {end_date}")
...
for url in urls:
    ...
    if rows:
        avg_pos = rows[0]['position']
        results_for_urls[url] = f"{avg_pos:.1f}"
    else:
        results_for_urls[url] = "-"
    
    print(f"{url}: {results_for_urls[url]}")

update_data = []
update_data.append([f"Avg Position ({latest_month})"])
...
result = sheets_service.spreadsheets().values().update(
    ...
    range=f"'{SHEET_NAME}'!T1:T{len(update_data)}",
    valueInputOption="USER_ENTERED",
    body=body
).execute()
print(f"Updated {result.get('updatedCells')} cells in Column T.")
