import os
from google.oauth2 import service_account
from google.analytics.data_v1beta import BetaAnalyticsDataClient
from google.analytics.data_v1beta.types import (
    DateRange, Dimension, Metric, RunReportRequest, FilterExpression, FilterExpressionList, Filter,
)

creds_info = {
    'type': 'service_account',
    'project_id': 'gen-lang-client-0042204106',
    'private_key_id': '800e02ba1d02083ec1711ff5f6e033618d050e84',
    'private_key': '-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n',
    'client_email': 'aiwar-841@gen-lang-client-0042204106.iam.gserviceaccount.com',
    'token_uri': 'https://oauth2.googleapis.com/token',
}

creds = service_account.Credentials.from_service_account_info(creds_info)
ga4_client = BetaAnalyticsDataClient(credentials=creds)
property_id = "292925407"

# Test 1: EXACT MATCH /vagina-filler/
print("Test 1: EXACT MATCH on '/vagina-filler/'")
req_exact = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="yearMonth")],
    metrics=[Metric(name="activeUsers"), Metric(name="userEngagementDuration")],
    date_ranges=[DateRange(start_date="2025-07-01", end_date="2025-07-31")],
    dimension_filter=FilterExpression(
        and_group=FilterExpressionList(
            expressions=[
                FilterExpression(
                    filter=Filter(
                        field_name="pagePath",
                        string_filter=Filter.StringFilter(value="/vagina-filler/"),
                    )
                ),
                FilterExpression(
                    filter=Filter(
                        field_name="sessionDefaultChannelGroup",
                        string_filter=Filter.StringFilter(value="Organic Search"),
                    )
                )
            ]
        )
    )
)
res = ga4_client.run_report(req_exact)
for row in res.rows:
    users = int(row.metric_values[0].value)
    dur = float(row.metric_values[1].value)
    print(f"  Exact: Users = {users}, Avg Eng = {dur/users:.2f}s (Total duration = {dur})")

# Test 2: CONTAINS /vagina-filler
print("\nTest 2: CONTAINS MATCH on 'vagina-filler'")
req_contains = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="yearMonth")],
    metrics=[Metric(name="activeUsers"), Metric(name="userEngagementDuration")],
    date_ranges=[DateRange(start_date="2025-07-01", end_date="2025-07-31")],
    dimension_filter=FilterExpression(
        and_group=FilterExpressionList(
            expressions=[
                FilterExpression(
                    filter=Filter(
                        field_name="pagePath",
                        string_filter=Filter.StringFilter(
                            value="vagina-filler",
                            match_type=Filter.StringFilter.MatchType.CONTAINS
                        ),
                    )
                ),
                FilterExpression(
                    filter=Filter(
                        field_name="sessionDefaultChannelGroup",
                        string_filter=Filter.StringFilter(value="Organic Search"),
                    )
                )
            ]
        )
    )
)
res = ga4_client.run_report(req_contains)
for row in res.rows:
    users = int(row.metric_values[0].value)
    dur = float(row.metric_values[1].value)
    print(f"  Contains: Users = {users}, Avg Eng = {dur/users:.2f}s (Total duration = {dur})")

# Test 3: sessionDefaultChannelGroup vs firstUserDefaultChannelGroup
print("\nTest 3: Channel Grouping check (sessionDefaultChannelGroup vs firstUserDefaultChannelGroup)")
# Let's check with firstUserDefaultChannelGroup
req_channel = RunReportRequest(
    property=f"properties/{property_id}",
    dimensions=[Dimension(name="yearMonth")],
    metrics=[Metric(name="activeUsers"), Metric(name="userEngagementDuration")],
    date_ranges=[DateRange(start_date="2025-07-01", end_date="2025-07-31")],
    dimension_filter=FilterExpression(
        and_group=FilterExpressionList(
            expressions=[
                FilterExpression(
                    filter=Filter(
                        field_name="pagePath",
                        string_filter=Filter.StringFilter(
                            value="vagina-filler",
                            match_type=Filter.StringFilter.MatchType.CONTAINS
                        ),
                    )
                ),
                FilterExpression(
                    filter=Filter(
                        field_name="firstUserDefaultChannelGroup",
                        string_filter=Filter.StringFilter(value="Organic Search"),
                    )
                )
            ]
        )
    )
)
res = ga4_client.run_report(req_channel)
for row in res.rows:
    users = int(row.metric_values[0].value)
    dur = float(row.metric_values[1].value)
    print(f"  FirstUser Channel: Users = {users}, Avg Eng = {dur/users:.2f}s (Total duration = {dur})")
